...
(() => {
  ...
  const EVENT_DATE = '2026-11-14T12:00:00+01:00';
  ...
  ============================================================================ */
  const i18nTargets = Array.from(document.querySelectorAll('[data-nl]')).map((el) => ({
    el,
    nl: el.dataset.nl,
    en: el.innerHTML,
  }));
  ...
  const setLang = (lang) => {
    currentLang = lang;
    i18nTargets.forEach(({ el, nl, en }) => { el.innerHTML = lang === 'nl' ? nl : en; });
    doc.lang = lang;
  ...
  const initCursor = () => {
    ...
    const moveRingX = gsap.quickTo(ring, 'x', { duration: 0.32, ease: 'power3.out' });
    ...
    document.querySelectorAll('a, button, .division, input, textarea').forEach((el) => {
  ...
  const heroRows = document.querySelectorAll('.hero__title .row > span, .page-hero h1 .row > span');
  ...
  const revealHero = () => {
    ...
    if (heroRows.length) {
      tl.from(heroRows, { yPercent: 110, duration: 1, ease: 'power4.out', stagger: 0.12 }, loader ? '-=0.25' : 0);
    }
    if (heroFades.length) {
      tl.from(heroFades, { y: 26, autoAlpha: 0, duration: 0.8, ease: 'power3.out', stagger: 0.1 }, '-=0.5');
    }
  };
  ...
  const setMenu = (open) => {
    ...
    if (open && hasGsap && !prefersReducedMotion) {
      ...
      gsap.fromTo('.menu__ctas, .menu__socials',
        { y: 20, autoAlpha: 0 },
  ...
  if (hasScrollTrigger && !prefersReducedMotion) {
    ...
    gsap.utils.toArray('[data-count]').forEach((el) => {
      ...
      gsap.to(state, {
        ...
        onUpdate: () => { el.textContent = `${Math.round(state.v)}${end >= 50 ? '+' : ''}`; },
      });
    });
  } else {
