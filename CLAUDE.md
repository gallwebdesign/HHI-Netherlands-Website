...
npm test             # build + Playwright smoke test — 57 passed with photos
npm run preview      # serves build/ (see the port-4173 trap below)
```

⚠️ **The count depends on whether the gallery photos are on your disk, and they are not in the repo** (see the gallery section below). Five tests carry a `test.skip` that fires when `static/img/gallery/` holds no images — the two lightbox tests, the paging one, the filename-encoding one and the thumbnail-split one. A fresh clone therefore reports fewer passes and some skips; **57 passed / 0 skipped is the with-photos number, verified 21 Aug 2026**, and the exact fresh-clone split has not been re-measured since the count changed. A *failure* in those five means something is broken; a *skip* means there are no photos there.
...
- The five photo-dependent smoke tests (lightbox ×2, paging, filename encoding, thumbnail split) **self-skip** when the folders are empty, so CI stays green. A skip means "no photos here", not "broken test".
...
- ⚠️ **The grid serves thumbnails, the lightbox serves the original — do not collapse the two.** Photos are ~1500×1000 at ~318KB and a grid tile renders about 209px wide, so the first screen of 24 was **6.6MB**, roughly 90% of it discarded by the browser after decoding. [static/api/thumb.php](static/api/thumb.php) serves a ~480px WebP (~29KB) generated on first request and cached under `img/gallery/2026/.thumbs/`; measured **6.6MB → 0.67MB**. `photo.thumb` feeds the tile, `photo.src` the lightbox. A smoke test asserts that split, because "simplifying" both back to `photo.src` silently restores the 6.6MB page.
- **thumb.php degrades to the original, never to a broken image.** Missing GD, an unwritable cache folder, a corrupt source — every failure path streams the full-size file instead of erroring, so deploying it cannot make the page worse than it was. Verified by running with `imagewebp` disabled: 200, `image/jpeg`, full size.
- Photos are served as dropped, with no resize step at upload. **Export at ~1600px long edge before adding them**, matching the 1200×900 discipline of `image01`–`08` — the thumbnail service handles the grid, but the lightbox still sends whatever you uploaded.
